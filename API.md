...
| 参数名 | 类型 | 必填 | 说明 |
|--------|------|------|------|
| requirement | string | 是 | 代码生成需求，长度1-1000字符 |
...
所有云函数接口均已添加输入验证，以下为统一验证规则：

### 用户相关验证

| 参数 | 验证规则 |
|------|----------|
| username | 长度3-20字符，仅允许字母、数字、下划线 |
| password | 长度6-20字符 |
| email | 符合邮箱格式 |
...
| message | 长度1-1000字符 |
| code | 长度1-10000字符 |
| requirement | 长度1-1000字符 |
| fileContent | 长度1-50000字符 |

### 安全机制

- **Token认证**：所有接口（register/login除外）均需提供有效的用户token
- **输入过滤**：对所有用户输入进行特殊字符过滤，防止注入攻击
- **参数类型校验**：对所有请求参数进行类型检查
- **长度限制**：对所有输入内容设置合理的长度上限
