...
项目已部署到 GitHub Pages，线上访问地址：

**https://wzyy886.github.io/AI123/**

> **说明**: 前端通过 GitHub Pages 托管，后端云函数部署在 uniCloud 阿里云服务空间，接口通过 uniCloud HTTPS 网关访问，无跨域问题。

### ☁️ 后端接口状态

后端云函数已部署在 uniCloud 阿里云服务空间：
- **服务空间**: mp-52179502-df44-46c2-a3c7-80d1ae48868a
- **云函数**: 10个云函数均已上传部署
- **数据库**: 5张数据表已创建
...
### GitHub Pages 部署（当前使用）

项目已通过 GitHub Pages 部署，步骤如下：
...
2. **推送 gh-pages 分支**
   ```bash
   # 使用 git subtree 推送
   git subtree push --prefix unpackage/dist/build/web origin gh-pages
   ```
...
4. **uniCloud 跨域配置**
   - 登录 uniCloud 控制台
   - 选择服务空间 → 跨域配置
   - 添加域名：`wzyy886.github.io`
   - 保存生效

### uniCloud 前端网页托管（备选）

> 注意：uniCloud 阿里云 OSS 存在策略限制，可能导致上传失败。推荐使用 GitHub Pages。

1. HBuilderX 中右键项目 → 发行 → 网站-PC Web 或手机 H5
2. 编译完成后自动上传到 uniCloud 前端网页托管
3. 在 uniCloud 控制台查看访问域名

### 小程序端部署

1. 配置小程序 AppID（在 [manifest.json](manifest.json) 中）
2. 微信开发者工具导入项目
3. 上传代码并提交审核
...
| [code_review.md](code_review.md) | AI 代码审查报告（Markdown版） |
| [prompt_log.md](prompt_log.md) | Prompt 日志（12条完整记录） |
| [summary.md](summary.md) | 个人实训总结报告（Markdown版） |
| [个人总结报告.docx](个人总结报告.docx) | 个人总结报告（Word版） |
| 数据库、接口截图包、AI Code Review/ | 数据库截图、接口截图、AI代码审查Word报告 |
